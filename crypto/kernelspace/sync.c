...
/*
 * In this example we're going to use an unusual cipher as an example:
 * salsa20.  I'm using it because I had a problem during my work related to
 * this guy and during the time I was testing it I figured that it would be a
 * nice article to write about crypto subsystem.
 *
 * Salsa20 was originally writen over blkcipher (synchronous multi-block
 * cipher) layer, which some time ago was deprecated in behalf of skcipher
 * (symetric key cipher) calls. Although you can see "type: blkcipher" in
 * /proc/crypto when you load salsa20 module, we're going to use skcipher type
 * calls. These new calls indentify the original type of the selected cipher
 * and within its logic make the changes between skcipher and blkcipher when
 * needed.
 */
